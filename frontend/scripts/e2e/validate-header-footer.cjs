...
(async () => {
  const url = process.env.EDITOR_URL || 'http://localhost:3000/editor-layout';
  ...
  try {
    ...
    await page.waitForFunction(() => {
      ...
      return !!h && h.textContent && h.textContent.trim().includes('Editor de Layout');
    }, { timeout: 10000 });
    ...
    (async () => {
      ...
      try {
        ...
        // Wait for editor title (localized)
        await page.waitForFunction(() => {
          const h = document.querySelector('h1');
