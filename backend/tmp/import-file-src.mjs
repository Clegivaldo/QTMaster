...
// Import directly from TS sources using tsx runtime (paths relative to /app when copied to container root)
const { enhancedFileProcessorService } = await import('./src/services/enhancedFileProcessorService.ts');
...
await main().catch(err => { console.error(JSON.stringify({ error: err?.message ?? String(err) })); process.exit(1); });
